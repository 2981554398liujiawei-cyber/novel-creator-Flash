...
def main() -> int:
    ...
    with workspace_lock(root):
        ...
        if first_heading(prose) != first_heading(old_prose):
            parser.error("prose-only rewrite must preserve the exact chapter heading; use a state-changing rewrite workflow otherwise")
        ...
        print(json.dumps({
            "rewritten": True,
            "chapter": chapter,
            "mode": "prose-replacement-pending-fact-review",
