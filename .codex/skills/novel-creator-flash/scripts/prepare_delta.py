...
def _safe_to_replace_existing(data: Any, chapter: int) -> bool:
    ...
    material = {
        ...
        for key in (
            ...
            "reader_expectation_added", "reader_model_updates", "entities", "events", "depends_on_events",
...
def main() -> int:
    ...
    provisional_path = safe_workspace_path(root, f".novel/staging/deltas/{chapter_meta_filename(args.chapter)}", allow_missing=True)
    provisional = load_json(provisional_path, default=None)
    if provisional is not None and (not isinstance(provisional, dict) or provisional.get("schema") != 1 or provisional.get("chapter") != args.chapter):
        parser.error("provisional staging delta is malformed; fix it or remove it before prepare-delta")
    delta = {
        "schema": 1,
    ...
    if isinstance(provisional, dict):
        # Preserve story discoveries made during drafting. Review-specific fields below are refreshed.
        for key in ("summary","outline_node","chapter_function","dominant_change","reader_expectation_added","reader_model_updates","entities","events","depends_on_events","knowledge_used","state_used","entity_changes","current_patch"):
            if key in provisional:
                delta[key] = provisional[key]
        delta["title"] = provisional.get("title") or delta["title"]
        if not isinstance(delta.get("current_patch"), dict):
            parser.error("provisional current_patch must be an object")
