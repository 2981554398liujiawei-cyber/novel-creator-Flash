...
from reader_model import apply_updates as apply_reader_model_updates, load_reader_model, validate_updates as validate_reader_model_updates
...
def validate_delta_shape(delta: dict[str, Any], chapter: int, prose: str) -> list[str]:
    ...
    errors.extend(validate_reader_model_updates(delta.get("reader_model_updates", [])))
...
def main() -> int:
    ...
    with workspace_lock(root):
        ...
        latest = latest_value
        reader_model_path = safe_workspace_path(root, "state/reader-model.json", allow_missing=True)
        try:
            reader_model = load_reader_model(root)
            next_reader_model = apply_reader_model_updates(reader_model, delta.get("reader_model_updates", []), chapter)
        except ValueError as exc:
            parser.error(str(exc))
        ...
        meta = {
            ...
            "reader_model_updates": delta.get("reader_model_updates", []),
        ...
        touched = [current_path, reader_model_path, final_path, meta_path, delta_path]
        ...
        try:
            ...
            atomic_write_json(reader_model_path, next_reader_model)
        ...
        provisional_path = safe_workspace_path(root, f".novel/staging/deltas/{chapter_meta_filename(chapter)}", allow_missing=True)
        if provisional_path.exists():
            provisional_path.unlink()
        try:
            from working_state import compile_working_state
            compile_working_state(root, write=True)
