...
from common import atomic_write_json, atomic_write_text, chapter_filename, load_json, normalize_entity_id, normalize_event_id, output_under, read_text, sha256_text, utc_timestamp, validate_workspace_layout, ensure_no_symlink_chain
...
def main() -> int:
    ...
    payload={"strong_affected_chapters":sorted(strong_reasons),"weak_affected_chapters":sorted(weak_reasons),"output":output.relative_to(root).as_posix()}
    if args.chapter:
        prose_path=root/'chapters'/chapter_filename(args.chapter)
        payload['chapter']=args.chapter
        if prose_path.is_file():
            payload['prose_sha256']=sha256_text(read_text(prose_path,required=True)); payload['generated_at']=utc_timestamp()
            receipt=root/'audits'/f'rewrite-impact-{args.chapter:04d}.json'; atomic_write_json(receipt,payload); payload['receipt']=receipt.relative_to(root).as_posix()
