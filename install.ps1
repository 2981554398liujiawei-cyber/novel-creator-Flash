...
try{
 ...
 Move-Item -LiteralPath $StagingSkill -Destination $TargetSkill; foreach($Name in $AgentNames){Move-Item -LiteralPath (Join-Path $StagingAgents $Name)-Destination (Join-Path $AgentsRoot $Name)}; Remove-Item -LiteralPath $StagingAgents -Force
 if($Migrate){foreach($Name in $LegacySkillNames){$p=Join-Path $SkillsRoot $Name;if(Test-Path -LiteralPath $p){Move-Item -LiteralPath $p -Destination (Join-Path (Join-Path $BackupRoot 'legacy-skills') $Name)}}; foreach($Name in $LegacyAgentNames){$p=Join-Path $AgentsRoot $Name;if(Test-Path -LiteralPath $p){Move-Item -LiteralPath $p -Destination (Join-Path (Join-Path $BackupRoot 'legacy-agents') $Name)}}}
}finally{if(Test-Path -LiteralPath $StagingSkill){Remove-Item -LiteralPath $StagingSkill -Recurse -Force -ErrorAction SilentlyContinue};if(Test-Path -LiteralPath $StagingAgents){Remove-Item -LiteralPath $StagingAgents -Recurse -Force -ErrorAction SilentlyContinue}}
